...
namespace ndof {
        ...
        private:
            ...
            struct TakeImpl<N,ArgumentHelper<T...>,Transform> {
                ...
                using type = TakeImpl< N, ArgumentHelper< std::tuple_element_t< current_size+1,std::tuple<T...>>,T...>, Transform >::type;
            };
        ...
        public:
            ...
            // Take the first element as a ArgumentHelper.
            using Car = Take<1>;

            // Take everything but the first element as a ArgumentHelper.
            ...
            constexpr auto take() const {
                return std::apply([](auto... elems) { return Take<N, Transform>{elems...}; }, *this);
            }

            // Take the last N elements and optionally apply a transform to them.
            template<std::size_t N, template<typename> typename Transform = Identity>
            constexpr auto take_back() const {
                return std::apply([](auto... elems) { return TakeBack<N, Transform>{elems...}; }, *this);
            }
